...
void loop(){
  ArduinoOTA.handle();
  /*long now = millis();
  //Alle X ms wird geprüft ob LEDS angesteuert werden müssen.
  if (now - lastMsg > updaterateLED) {  
    if(button_pressed){fade_white(weiss_helligkeit);}
    else{//Ab hier ausschalten
      //fade_white(0);
      if (zielwert_erreicht==1){
        if(i<=NUM_LEDS){
          fade_white_single(weiss_helligkeit, 0, i-1);
        }
        else{ //zum automatischen WiederEinschalten --- nur Test
          button_pressed = 1;
          hue = 0;
          i = 0;
          fill_solid(leds, NUM_LEDS, CRGBW(250,0,0,0));
  ...
  //Hilfvariable zum Ausschalter Drücken Simulieren
  if (now - lastMsg2 > ausschalten) {  
    button_pressed = 0;
